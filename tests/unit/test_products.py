...
from src.build3.acquire import weather
from src.build3.derive import products, zonal
...
class TestClosures:
    def test_crops_closure_passes_and_fails(self):
        cls = [f"{c.lower()}_px" for c in products.cdl_classes.CLASSES]
        good = pd.DataFrame({c: [1.0] for c in cls} | {"comid": [1], "year": [2020],
                                                       "n_px": [float(len(cls))]})
        ok, _ = products._crops_closure(good, None)
        assert ok
        bad = good.assign(n_px=999.0)
        ok, note = products._crops_closure(bad, None)
        assert not ok and "1 with class areas" in note
