...
def _path(name: str):
    return config.COMID_ATTRS / f"{name}.parquet"
...
def _write(df: pd.DataFrame, name: str) -> int:
    config.COMID_ATTRS.mkdir(parents=True, exist_ok=True)
    out = df.copy()
...
def wieczorek(comids: list[int], force: bool = False, batch: int = 100) -> int:
    ...
    for theme in WIECZOREK_THEMES:
        for wtype, prefix in WIECZOREK_TYPES.items():
            ...
            try:
                d = config.COMID_ATTRS / f"wieczorek_{slug}"
...
def nid(force: bool = False) -> int:
    ...
    cache = config.DATA.parent / "cache" / "nid_national.gpkg"
