...
def _pending_path(year: int, month: int):
    ...
    return config.WEATHER / "_pending" / f"gridmet_{year}-{month:02d}.parquet"
...
def _quarter_path(year: int, q: int):
    return config.WEATHER / f"gridmet_{year}Q{q}.parquet"
...
def _grid_path():
    return config.WEATHER / GRID_PATH_NAME
...
def tiles():
    ...
    out = [tuple(b) for b in config.seed_cover()]
...
def read_cells(cells, start=None, end=None):
    ...
    out = []
    for p in sorted(config.WEATHER.glob("gridmet_*Q*.parquet")):
        f = pq.ParquetFile(p)
        md = f.metadata
...
def fetch_month(year: int, month: int, keep: set[int], force: bool = False) -> int:
    ...
    for bx in tiles():
        try:
            ...
            # NCSS answers 400 for a window past the end of the record. That is "not published yet", not a failure -- the loop runs to YEAR_END, and the last months of the final year do not exist until they happen.
            if e.code == 400:
                return -1
            raise
...
def main(force: bool = False, years: range | None = None) -> dict:
    ...
    STOPS AT THE END OF THE RECORD rather than at YEAR_END. The declared span runs to 2026, but only part of 2026 has happened; asking NCSS for a future month earns a `400 Bad Request` that used to take the whole branch down after every real month had already been fetched.
    """
    grid = aoe_cells(force=force)
    keep = set(grid.cell_id.tolist())
    print(f"  {len(keep):,} gridMET cells intersect the AOE", flush=True)

    years = years or range(config.YEAR_START, config.YEAR_END + 1)
    written = {}
    for y in years:
        for m in range(1, 13):
            t0 = time.time()
