...
WHY LOCAL AND NOT NLDI. 388 gauges is 388 requests to a service that has already answered 5,063,593 km2 for a Cape Cod estuary. The catchments and the value-added attributes are both on disk, the walk is 4 seconds for the whole cohort, and it agrees with StreamCat's `ws` accumulation by construction because it is the same traversal over the same topology.
...
VERIFIED AGAINST NHDPlus ITSELF. `vaa.totdasqkm` is the total drainage area NHDPlus computed for each reach, by its own accumulation, independently of ours. Summing `areasqkm` over the walked set reproduces it to within 1% on 388 of 392 sites. That is the check that makes this trustworthy rather than plausible, and `flag_area_vs_vaa` keeps it running on every build.

THE FOUR THAT FAIL ARE THE FOUR THE WHOLE PIPELINE KEEPS TRIPPING OVER. Orient Harbor, Hog Island Channel, Mattapoisett Harbour and Sippican Harbour snap to `Coastline` reaches, and a coastline is not a drainage line: walking upstream from one collects every river that reaches the sea along it -- 124,184 reaches and 251,225 km2 for a site whose own reach drains 20 km2. NLDI does the same thing and returns the same nonsense, which is where its 5M km2 answers come from. `NON_DRAINAGE_FTYPE` refuses the walk instead.
"""
...
# Reach types that do not carry drainage upstream. Walking from one is not an approximation, it is a category error: a coastline connects places that share a shore, not places that share a watershed.
...
class Network:
    ...
    Holds three dicts and no geometry: adjacency by node, each reach's upstream node, and the per-reach areas. About 1 s to build and ~200 MB, against a 1.25 GB flowline layer that never has to be opened.
    """
