...
Everything is already on disk. The registry is `raw/water/site_clean.csv`; the observations are the 85-file, 42,394,345-row CSV export under `raw/water/chunks/`. This adapter reads both and re-downloads nothing.
...
The split is materialised ONCE PER BUILD into `interim2/`, not per site: all 85 files read together take about five seconds with `pyarrow.csv`, while 135 independent scans of 3.1 GB would not be affordable. It is rebuilt when any chunk is newer than the split.
...
The registry contains junk ids -- JIMTEST, UNKNOWN, bare USGS station numbers -- so candidates are the uids matching `WQS\\d+` or `WQP\\d+`. That is a namespace rule, not a quality filter: no site is dropped for being short, sparse or odd. `WQP` is IIHR's conservation-practice programme -- saturated buffers, blind inlets, in-field plots and a handful of river stations -- and it INVERTS THE WQS NAMING CONVENTION: `river` holds the watercourse and `nickname` holds what the station is, so `station_name` keeps both or the only evidence of a type is lost.

THE EXPORT AND THE REGISTRY DISAGREE, AND EACH IS AUTHORITATIVE FOR A DIFFERENT THING -- the export for observations, the registry for location. `materialise` filters the export and `discover` filters the registry, so a station in one and not the other falls between them: `WQP0016` carries 13,644 nitrate observations and has no registry row, hence no coordinates, hence no registration `schema.validate` would accept. `reconcile` exists so that is reported rather than silent, which is exactly the loss the split was introduced to prevent.
"""
...
_REGISTRY = config.RAW_WATER / "site_clean.csv"
_CHUNKS = config.RAW_WATER / "chunks"                  # the export; the authority
_SPLIT = config.INTERIM / "water" / "iwqis"            # per-site view, rebuilt from it
...
# TRACKED, and it sits beside the adapter rather than under `interim2/` because it is a statement about the
# SOURCE, not about a build. See `reconcile`.
_GAPS = Path(__file__).with_name("iwqis-registry-gaps.csv")
...
def materialise(force: bool = False) -> int:
    ...
    for uid, grp in big.groupby("site_uid", sort=True):
        ...
        bio.write_parquet(d, _SPLIT / f"{uid}.parquet")
    stale = [p for p in _SPLIT.glob("*.parquet") if p.stem not in set(big.site_uid)]
    for p in stale:
        p.unlink()
...
def discover() -> pd.DataFrame:
    """Every IIHR-registered station inside the AOE, from the registry rather than from what happens to have a parquet."""
    reconcile()
    r = _registry()
...
def reconcile(verbose: bool = True) -> list[str]:
    ...
    ack = set()
    if _GAPS.exists():
        ack = set(pd.read_csv(_GAPS, dtype=str).uid.astype(str).str.strip().str.upper())
    if verbose:
        ...
        if fresh:
            print(f"    NOT ACKNOWLEDGED: {', '.join(fresh)} -- the registry has moved; refetch it, or add "
                  f"the id to {_GAPS.name} with a reason")
    return gaps
