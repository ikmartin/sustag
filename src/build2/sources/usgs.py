...
def _tier_pcodes(channels: str | None) -> list[str]:
    ...
    if not channels or channels != channels:
...
def discover(census: bool = True) -> pd.DataFrame:
    ...
    per = per.join(nagg, on="monitoring_location_id")
...
def fetch(native_id: str, start=None, end=None, min_nitrate_days: int = 0,
          tier: str = "native", channels: str | None = None) -> pd.DataFrame | None:
    ...
    min_nitrate_days : int, default 0
        Skip the covariate passes when the site has fewer than this many nitrate days. A COST GUARD ONLY -- the frame still comes back and `records.py` owns the decision to keep the site.
    tier : {'native', 'dv'}, default 'native'
        `dv` fetches the daily mean alone and never asks what the site reports; see `params.fetch_tier`.
    channels : str, optional
        `'+'`-joined canonical channels the site declares. Restricts a `dv` fetch to the pcodes behind them.
    ...
    if out is None:
        return None
    if min_nitrate_days and _standard_days(out.index) < min_nitrate_days:
        return out
