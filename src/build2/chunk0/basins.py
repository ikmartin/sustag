...
from ..chunk3.site_type import prior_no_surface
...
def fetch_seed_basins(sites: pd.DataFrame, force: bool = False, workers: int = _WORKERS) -> pd.DataFrame:
    ...
    config.NLDI_BASINS.mkdir(parents=True, exist_ok=True)
    seeds = sites[sites.aoe_seed.fillna(False)]
    todo, cached = [], []
    for r in seeds.itertuples():
        p = config.NLDI_BASINS / f"{schema.uid_to_filename(r.site_uid)}.parquet"
