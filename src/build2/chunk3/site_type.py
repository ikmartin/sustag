"""What a site IS: stream, lake, well, tile outlet. Decided here, in chunk 3, not at discovery.

WHY NOT CHUNK 0. Chunk 0 has exactly two inputs -- the source's own type field, and a regex over the station name -- and for 152 of 392 registrations the regex matches nothing at all. Those are `*_default_stream`: called a stream because nothing said otherwise. It cannot do better, because the evidence that settles the question is the reach the site snaps to and the waterbody that reach threads, and neither exists until chunk 1 downloads the network and chunk 3 computes the snap.

`IWQIS:WQS0116 Iowa Great Lakes 1 at Spirit Lake` is the case that motivates this. It is typed `stream` by fall-through, sits inside a `LakePond` polygon, and `lake` is in `NO_SURFACE_BASIN` -- so today it enters the trainable cohort as a stream gauge. `chunk0/basins.py` names the trap exactly: the regex is deliberately conservative because "'Spirit Lake' cannot be told from 'Storm Lake' by pattern alone." One is a lake; one is a creek at a town called Storm Lake. No amount of metadata separates them. A polygon does.

WHAT THE NHD EVIDENCE IS ACTUALLY WORTH, measured on this cohort rather than assumed:

  FTYPE == Coastline        4 sites, precision 1.00, catches 4 of 7 estuaries
  WBAreaType == LakePond   12 sites, precision 0.50 -- 6 lakes and 6 WIDE RIVERS
  WBAREACOMI > 0          107 sites -- not a signal, but a JOIN KEY to the real polygon
  Tidal == 1               17 sites, ~0.35 -- all 3 tidal_stream, 3/7 estuary, 9 false positives
  well / tile / ditch / pond              nothing at all: 8 of 13 wells snap to StreamRiver

Two things that look like signals and are not. `FTYPE == CanalDitch` does NOT mean the site is a ditch -- 9 reaches in the cohort carry it and 3 of them are wells; it means the reach is canalised, which for `Little Sioux River at Turin` says the river is leveed there. And `WBAreaType` at 0.50 precision cannot classify on its own: it would call the Susquehanna, the Maumee and the Illinois at Peoria lakes. The upgrade is not the flowline attribute, it is that `WBAREACOMI` lets us fetch the actual polygon and test containment.

RANK PROMOTES, IT DOES NOT OVERRIDE. A site typed by its source's own field or by a strong name match keeps that type; where the NHD disagrees the disagreement is recorded in `site_type_conflict` and nothing changes. That is what stops the wide rivers being reclassified and dropped. The 152 fall-through defaults have no such evidence, so the NHD resolves them outright -- which is where Spirit Lake gets caught. Everything ambiguous goes to `site-type-review/` for a human.
"""
...
def prior_no_surface(source: str, raw, name) -> bool:
    """Can this site NOT have an upstream surface basin? The only typing chunk 0 needs.

    Standing and tidal water does not drain from anywhere, so snapping one to a reach hands it a basin belonging to something else -- a category error, not an approximation. Chunk 0 calls this before fetching NLDI basins, which is three steps before the AOE is unioned, so it cannot wait for the snap.

    A PRIOR, NOT THE ANSWER. It sees only metadata, so it is exactly today's chunk-0 classifier restricted to the question that matters there. It is not the only protection either: `MAX_CONUS_KM2` plus `_flag_oversized` exclude oversized basins from the union afterwards, and every impossible basin chunk 0 has produced -- four estuaries at 5.01-5.06M km2 -- exceeds that cap. This saves ~35 pointless fetches and catches the narrower case the cap cannot see, a lake handed a plausibly-sized basin that is not its drainage.
    """
...
WATERBODIES = config.NETWORK / "waterbodies.parquet"

# Flowline attributes the classifier reads. `WBAREACOMI` is the join key to the polygon; the rest are the
# weaker corroborating signals whose measured precision is in the module docstring.
...
def waterbodies(wb_comids, force: bool = False):
    """NHD waterbody polygons for the given waterbody COMIDs, EPSG:5070. Cached.

    FETCHED BY ID, NOT IN BULK. `nhdplus_l48(layer="NHDWaterbody")` is a 7.3 GB refetch -- pynhd unlinks the archive after extraction and `cache/` holds no `.gdb` -- for the ~78 polygons this cohort actually references. `WaterData("nhdwaterbody").byid` retrieves exactly those, the same per-site WFS pattern `basins.snap_comid` already uses.

    PARTIAL FAILURE IS EXPECTED AND FINE. A `WBAREACOMI` may reference an `NHDArea` (a wide-river polygon) rather than an `NHDWaterbody`, and those ids simply return nothing. A missing polygon means no rank-3 evidence for that site, which leaves it on whatever the earlier ranks decided -- never worse than before. The whole service being down must degrade the same way: losing 152 fall-through sites to an outage would be far worse than the guess they already carry.
    """
    import geopandas as gpd

    if WATERBODIES.exists() and not force:
        return gpd.read_parquet(WATERBODIES)

    ids = sorted({int(w) for w in wb_comids if pd.notna(w) and int(w) > 0})
    if not ids:
    ...
    try:
        ...
        print(f"  waterbody service unavailable ({type(e).__name__}) -- continuing without rank-3 evidence")
        return gpd.GeoDataFrame({"comid": []}, geometry=[], crs=config.EQUAL_AREA)
...
def load_decisions() -> pd.DataFrame:
    """Human type decisions, blank rows dropped. A blank `decision` means NOT YET REVIEWED, not an error.

    Lives here rather than in the review module because `classify` has to apply them, and the review module imports this one -- the other direction would be a cycle.
    """
    p = config.TYPE_DECISIONS
    if not p.exists():
        return pd.DataFrame(columns=["site_uid", "decision"])
    d = pd.read_csv(p, dtype=str).fillna("")
    val = d.decision.str.strip().str.lower()
    bad = set(val[val != ""]) - set(VALID_TYPES)
    if bad:
        raise ValueError(f"{p.name} has unrecognised types {sorted(bad)}; expected one of {sorted(VALID_TYPES)}")
    out = d[val != ""].copy()
...
def classify(sites: pd.DataFrame, force: bool = False) -> pd.DataFrame:
    ...
    water = waterbodies(m.WBAREACOMI, force=force)
