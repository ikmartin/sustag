...
WHICH CHANNEL DECIDES. Nitrate where both have it, then discharge, then the densest shared channel. The level test is absolute for nitrate (mg/L as N, the calibrated threshold) and relative to the pair's own median for everything else, since 0.25 units means nothing shared between discharge in m3/s and pH.
...
# Which statistic represents a channel's day when two records are compared.
COMPARE_STAT = {"nitrate": "max"}
DEFAULT_STAT = "mean"
...
# TURBIDITY IS SEASONAL EVIDENCE TOO, and it was in `CORRELATION_PREFERENCE` above these, which meant it
# was preferred over the strength filter rather than caught by it. Two sensors in one stream agree on
# turbidity because they share a hydrograph -- it is storm-driven and basin-wide, exactly like the four
# below. The case that exposed it: two in-field instruments 8.3 m apart, one measuring nitrate and the
# other only turbidity, were judged on their turbidity correlation instead of reaching `complementary`,
# which is the branch a shared structure with disjoint instruments is supposed to take. Measured before
# moving it: of 21 pairs on the current cohort, ZERO are decided on a turbidity channel, so no recorded
# decision is disturbed.
WEAK_FOR_IDENTITY = frozenset({"water_temp", "gage_height", "ph", "diss_oxy",
...
# leaves two nodes with near-identical basins, which `chunk4.graph.edge_table` reports as
...
PAIR_COLUMN = "Merge Pair"
# THE THREE EDITABLE COLUMNS SIT TOGETHER, immediately after the pair they decide and before the evidence.
# A reviewer types in exactly these three; putting them last means scrolling past nine read-only columns to
# reach the one field that matters, on every row.
DECISION_COLUMNS = [PAIR_COLUMN, "uid_a", "uid_b",
                    "decision", "decided_by", "note",
                    "name_a", "name_b", "proposed", "sep_m", "on_channel", "shared_days", "r",
                    "channels_a", "channels_b"]
VALID_DECISIONS = {"merge", "distinct"}
...
def channels_of(frame: pd.DataFrame | None) -> list[str]:
    ...
    if frame is None or not len(frame):
        return []
    return [c for c in params.NAMES
...
def _series(frame: pd.DataFrame | None, channel: str) -> pd.Series | None:
    ...
    if frame is None:
        return None
    col = f"{channel}_{COMPARE_STAT.get(channel, DEFAULT_STAT)}"
...
def _agreement(sa: pd.Series, sb: pd.Series, channel: str) -> tuple[float, float, float, bool]:
    ...
    The level test is ABSOLUTE for nitrate, in mg/L as N, because that is the calibrated threshold; for every other channel it is RELATIVE to the pair's own median, since 0.25 units means nothing shared between discharge in m3/s and pH.
    """
...
def candidates(sites: pd.DataFrame) -> pd.DataFrame:
    ...
    A REGISTRATION WITH NO NITRATE RECORD IS NOT ELIGIBLE EITHER. It cannot be judged -- every branch here rests on comparing two records, so a pair missing one comes back `unknown_no_records` and sits pending forever -- and merging it would change nothing, because it contributes no rows whether grouped or not. On this cohort that is 50 of 392 registrations, and they hold no nitrate at all rather than a short record: chunk 1 now archives anything with a single nitrate day, so an absent file means the source published none.

    A SHORT record is a different case and IS eligible. Nine IWQIS sites carrying 1 to 42 days were withheld by the old 60-day collection floor, and four of them share a reach with a gauge that clears the bar -- days that extend a real record, invisible while the floor discarded them at fetch time. Whether a thin registration survives to `sites.parquet` is chunk 6's `MIN_GAUGE_DAYS`, applied to the MERGED record; here it only has to be judgeable.
...
def carries_nitrate(col) -> pd.Series:
    """Which side of each pair reports nitrate, read off the '+'-joined channel list."""
    return col.fillna("").astype(str).str.split("+").apply(lambda v: "nitrate" in v)
...
def blocks(cand: pd.DataFrame) -> pd.Series:
    ...
    into one gauge and when at least one side carries nitrate -- the two facts that decide whether a wrong
    ...
    wrong the result is two nodes with near-identical basins, which chunk 4 reports as `near_duplicate_basin`
    ...
    nitrate = carries_nitrate(cand.channels_a) | carries_nitrate(cand.channels_b)
    return nitrate & cand.proposed.isin(MERGE_PROPOSALS)
...
def sync_decisions_file(cand: pd.DataFrame) -> tuple[int, int, int]:
    """Reconcile `merge_decisions.csv` with the current candidates. Returns (added, kept, retired).

    Written with `decision` BLANK for a new pair, never guessed at from `proposed`. A pre-filled verdict is one a reviewer scrolls past, and the gate exists so a merge is asserted by a person rather than inherited from a threshold. Existing decisions are carried forward on the uid pair, so re-running never costs work already done.
    """
    from .. import _panel

    if not len(cand):
        return 0, 0, 0
    return _panel.sync_sheet(config.MERGE_DECISIONS, order_for_review(cand, load_decisions()),
                             ["uid_a", "uid_b"], PAIR_COLUMN, DECISION_COLUMNS)


def nitrate_days(sites: pd.DataFrame) -> pd.Series:
    """Nitrate days per uid, for picking the canonical member of a merged group.

    Prefers the record block's `n_nitrate_days` and falls back to counting rows in the daily parquet, so identity can run standalone before the record columns are joined on. A uid with neither is 0, which sorts it last.
    """
    have = (pd.to_numeric(sites.n_nitrate_days, errors="coerce")
            if "n_nitrate_days" in sites.columns else pd.Series(np.nan, index=sites.index))
    out = {}
    for uid, n in zip(sites.site_uid, have):
        if pd.notna(n):
            out[uid] = int(n)
            continue
        p = config.WATER_DAILY / f"{schema.uid_to_filename(uid)}.parquet"
        out[uid] = len(pd.read_parquet(p, columns=["date"])) if p.exists() else 0
    return pd.Series(out, dtype="int64")


def load_decisions() -> pd.DataFrame:
    """Human merge decisions, blank rows dropped.

    A BLANK `decision` MEANS NOT YET REVIEWED, not an error. The file is meant to be used as a worksheet -- generated from the candidate list with one row per pair and the column empty, then filled in over several sittings -- so a half-finished file has to load. Only a value that is neither blank nor recognised is a mistake worth stopping for.
    """
    p = config.MERGE_DECISIONS
    if not p.exists():
        return pd.DataFrame(columns=DECISION_COLUMNS)
    d = pd.read_csv(p, dtype=str).fillna("")
    val = d.decision.str.strip().str.lower()
    bad = set(val[val != ""]) - VALID_DECISIONS
    if bad:
        raise ValueError(f"{p.name} has unrecognised decisions {sorted(bad)}; expected {sorted(VALID_DECISIONS)}")
    return d[val != ""].reset_index(drop=True)


def apply_decisions(sites: pd.DataFrame, cand: pd.DataFrame, dec: pd.DataFrame) -> pd.DataFrame:
    """Group by connected component over CONFIRMED merges only, and stamp the merge block.

    The canonical member of a group is the one with the MOST nitrate days, with `schema.CANONICAL_PRECEDENCE` breaking ties and the uid breaking those. Source precedence alone is the wrong rule: on the Cedar River the MPCA and USGS registrations of one gauge differ by years of record, and naming the group after the shorter series would hide the longer one behind a non-canonical uid.
    """
    import networkx as nx

    out = sites.copy()
    confirmed, by_rule = set(), set()
    if len(dec):
        m = dec[dec.decision.str.strip().str.lower() == "merge"]
        confirmed = {tuple(sorted((a, b))) for a, b in zip(m.uid_a, m.uid_b)}
    rejected = set()
    if len(dec):
        rejected = {tuple(sorted((a, b))) for a, b in zip(dec.uid_a, dec.uid_b)} - confirmed
    if len(cand):
    ...
    prec = out.set_index("site_uid").source.map(schema.CANONICAL_PRECEDENCE).fillna(99)
    days = nitrate_days(out)
    group_of, canon_of = {}, {}
    for comp in nx.connected_components(g):
        canon = min(comp, key=lambda u: (-int(days.get(u, 0)), prec.get(u, 99), u))
        for u in comp:
            group_of[u], canon_of[u] = canon, canon

    out["physical_gauge_group"] = out.site_uid.map(group_of)
    # NULLABLE, because the next block writes pd.NA into it for a site in an undecided pair. A numpy bool
    # column raises `Invalid value 'nan' for dtype 'bool'` on that assignment -- and only when something is
    # actually pending, so the build ran green for as long as every pair happened to be decided.
    out["is_group_canonical"] = pd.array(out.site_uid == out.site_uid.map(canon_of), dtype="boolean")

    ev = cand.set_index(["uid_a", "uid_b"]) if len(cand) else None
    gate_of = ({tuple(sorted((a, b))): bool(g) for a, b, g in zip(cand.uid_a, cand.uid_b, blocks(cand))}
               if len(cand) else {})
    decided = {tuple(sorted((a, b))) for a, b in zip(dec.uid_a, dec.uid_b)} if len(dec) else set()

    # A SITE IN AN UNDECIDED PAIR HAS NO GROUP YET. Connected components over confirmed merges alone makes every unreviewed site a singleton, which for a site with no candidates is the truth -- nothing was proposed, so it stands alone. For a site sitting in a pending pair it is a false assertion, and specifically the one the contract warns about: both halves of a duplicate would carry `is_group_canonical = True` and anything trusting the flag double-counts. Null is what "not yet decided" looks like.
    if len(cand):
        gate = blocks(cand)
        pending_uids = {u for pair, g in zip(zip(cand.uid_a, cand.uid_b), gate)
                        if g and tuple(sorted(pair)) not in decided for u in pair}
        undecided = out.site_uid.isin(pending_uids)
        out.loc[undecided, "physical_gauge_group"] = pd.NA
        out.loc[undecided, "is_group_canonical"] = pd.NA
        if undecided.any():
            print(f"  {int(undecided.sum())} site(s) in an undecided pair: grouping left NULL")
    for col in ("merge_rule", "merge_decided_by"):
        out[col] = out[col].astype("object")
    out["merge_rule"] = "singleton"
    out["merge_decided_by"] = "n/a"

    if ev is not None:
        for (ua, ub), row in ev.iterrows():
            key = (ua, ub)
            state = "human" if key in decided else ("rule" if key in by_rule else
                                                    "rule" if not gate_of.get(key, True) else "pending")
            for u, other in ((ua, ub), (ub, ua)):
                sel = out.site_uid == u
                if not sel.any():
                    continue
                out.loc[sel, "merge_rule"] = row.proposed if key in confirmed else (
                    "distinct" if key in decided or not gate_of.get(key, True)
                    else row.proposed + "_pending")
                out.loc[sel, "merge_decided_by"] = state
                out.loc[sel, "merge_sep_m"] = row.sep_m
                out.loc[sel, "merge_shared_days"] = row.shared_days
                out.loc[sel, "merge_r"] = row.r
                out.loc[sel, "merge_median_diff"] = row.median_diff
                out.loc[sel, "merge_seam_date"] = row.seam
...
def main(sites: pd.DataFrame | None = None) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if len(cand):
        ...
        # the panel is drawn from it, so row N of one is row N of the other; `merge_auto.csv` is the record
        # of what the rule settled unasked, which is not a queue but must not be invisible either. The
        # per-site view of the same fact is `merge_rule` and `merge_decided_by` on the registration table.
        n_auto_merge = int(auto.proposed.isin(MERGE_PROPOSALS).sum())
        print(f"  {len(auto)} pair(s) settled by rule ({n_auto_merge} merged, "
              f"{len(auto) - n_auto_merge} left apart) -- no nitrate at stake or nothing to collapse")
        bio.write_csv(auto, config.MERGE_AUTO)
        decided = {tuple(sorted((a, b))) for a, b in zip(dec.uid_a, dec.uid_b)} if len(dec) else set()
        pending = [k for k in zip(ask.uid_a, ask.uid_b) if tuple(sorted(k)) not in decided]
        print(f"  {len(ask)} pair(s) need a person; decisions on file: {len(dec)}   PENDING: {len(pending)}")
        bio.write_csv(ask, config.MERGE_CANDIDATES)
        print(f"  wrote {config.MERGE_CANDIDATES.name} and {config.MERGE_AUTO.name}")

    out = apply_decisions(sites, cand, dec)
    n_groups = out.physical_gauge_group.nunique()
    n_undecided = int(out.physical_gauge_group.isna().sum())
    print(f"\n{len(out)} registrations -> {n_groups} physical gauges "
          f"({len(out) - n_groups - n_undecided} merged away, {n_undecided} awaiting a decision)")
    return out, cand
