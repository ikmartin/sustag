...
WHY A FIGURE AND NOT A TABLE. `merge_candidates.csv` already carries the evidence, but deciding a pair from it means holding a map, two records and eight numbers in your head simultaneously. A merge changes the unit of analysis and a wrong one corrupts basins, graph edges and the concatenated record at once, silently -- so the 36 pairs are worth per-pair review, and 36 is small enough to make that affordable.
...
STYLE FOLLOWS `src/build/water/filter_sites.py::render_panel`, the repo's existing long review panel, down to the inches-not-fractions title band and the monospace stats block. See `_figure` for why the fraction form breaks on a tall figure.
"""
...
from .._panel import A_COLOR, B_COLOR, INK, INK_2, MUTED, fmt
...
# Row order comes from `identity.order_for_review` so panel row N is worksheet row N.
BRANCH_ORDER = identity.REVIEW_ORDER



def out_dir():
    return _panel.review_dir("merge-review")


# ---- data -----------------------------------------------------------------------------------------

def _series_by_uid(uids) -> dict[str, pd.Series]:
    """Daily record per REGISTRATION, read straight from `interim2/water/daily/`.

    Registration-level on purpose. A merge review needs both halves of a pair as separate series -- comparing them IS the review -- so it cannot read an identity-unified table, where a merged registration has already been absorbed and a rejected one may be absent entirely. Only the uids under review are opened, which is 58 files rather than 333.
    """
    out = {}
    for u in uids:
        s = _panel.daily_record(u)
        if s is not None:
            out[u] = s
    return out
...
def pair_stats(row, sites: pd.DataFrame, series: dict[str, pd.Series]) -> dict:
    ...
    sa, sb = series.get(ua), series.get(ub)
...
def _legend(axes_row, cand) -> None:
    ...
    The same reasoning as the site-type panel's legend -- `decision` takes `merge` or `distinct` and nothing else, and a reviewer should not have to remember which threshold produced a proposal. The thresholds are read from `identity` rather than retyped, so the legend cannot drift from the rule it describes.
    """
    for ax in axes_row:
        ax.axis("off")
    ax = axes_row[2]
    ...
    body = "\n".join([
        ...
        f"  complementary       {n('complementary'):>3}   no shared CHANNEL, within "
        ...
        f"  dual_registration   {n('dual_registration'):>3}   overlap, r >= {identity.DUAL_MIN_R} AND |median diff| <= "
        f"{identity.DUAL_MAX_MEDIAN_DIFF} mg/L (nitrate; relative elsewhere)",
...
def _draw_map(ax, st, catch, reaches, pts):
    """Both registrations on the reach they share, with the catchment outlined."""
    _panel.draw_map(ax, pts, ["A", "B"], [A_COLOR, B_COLOR],
                    catch=catch, reaches=reaches, snapped=st["comid"])
...
def _stats_text(st) -> str:
    ...
    same_snap = "SAME" if st["comid_a"] == st["comid_b"] else "DIFF"
    same_cat = "SAME" if st["same_catchment"] else "DIFF"
    ov = "none" if st["shared_days"] in (0, None) else f"{st['shared_days']:,} d"
    flag = lambda v: {True: "y", False: "NEAR-DIVIDE"}.get(v, "?")
    far = lambda s: "" if s == "ok" else "  FAR"
    return "\n".join([
...
def main(force: bool = False) -> pd.DataFrame:
    ...
    if not config.MERGE_CANDIDATES.exists():
        raise FileNotFoundError(f"{config.MERGE_CANDIDATES} missing -- run chunk 3 first")
    # The SAME decisions the sheet was synced against, or panel row N stops being sheet row N.
    cand = identity.order_for_review(pd.read_csv(config.MERGE_CANDIDATES), identity.load_decisions())
    sites = pd.read_parquet(config.REGISTRATIONS_PATH)
    uids = set(cand.uid_a) | set(cand.uid_b)
    series = _series_by_uid(uids)
    print(f"  {len(cand)} candidate pair(s); {sum(u in series for u in uids)}/{len(uids)} have records")

    si = sites.set_index("site_uid")
    need = set()
    for c in ("comid", "catchment_comid"):
        need |= {int(v) for v in si.loc[list(uids), c].dropna()}
    ...
    draw = _panel.cap(cand, "candidate pair(s)", config.MERGE_DECISIONS)
    fig, axes = _panel.figure(len(draw) + 1)
    _legend(axes[0], cand)
    rows = []
    for i, row in enumerate(draw.itertuples(), start=1):
        st = pair_stats(row, sites, series)
        rows.append(st)
        ...
        _draw_map(ax_map, st, sub, reaches, [pts[st["uid_a"]], pts[st["uid_b"]]])
        _draw_series(ax_ts, series.get(st["uid_a"]), series.get(st["uid_b"]), st)
    ...
    w, h = _panel.finish(fig, len(draw) + 1,
                         ...
                         f"({n_seq} sequential, decided in merge_decisions.csv)", png)

    stats = pd.DataFrame(rows)
    stats.to_csv(d / "merge_review_stats.csv", index=False)
