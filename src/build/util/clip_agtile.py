...
AgTile-US (Valayamkunnath et al. 2020, Sci Data 7:257; Figshare DOI 10.6084/m9.figshare.11825742)
is a 30 m binary tile-drained-cropland raster: 1 = tile-drained, 0 = undrained cropland, and
NON-AGRICULTURAL land is MASKED (nodata), NOT coded 0. Single vintage 2017. CONUS uncompressed is
~14.9 GB, so -- exactly like the national CDL tifs in clip_crops -- the national file is downloaded
MANUALLY and this tool windows it down to the region once.

LICENCE: CC-BY 4.0 -- attribute Valayamkunnath, Barlage, Chen, Gochis & Franz (2020). Self-host the
Figshare TIFF; do not use the Google Earth Engine mirror for a deployed product.
...
def clip_agtile(force: bool = False) -> Path:
    ...
    minlon, minlat, maxlon, maxlat = get_region_bbox()  # WGS84
