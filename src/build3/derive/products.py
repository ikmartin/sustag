"""D5 -- every raster becomes a COMID-keyed table, through ONE driver that checks closure before it writes.
    ...
    python -m src.build3.run d5 --single
    python -m src.build3.run d5 --single --only weights,dams   # a product subset
    python -m src.build3.run d5 --single --force               # ignore every checkpoint
...
from . import archive, cdl_classes, dams, zonal
...
CDL_CRS = "EPSG:5070"
YEAR_MIN, YEAR_MAX = config.YEAR_START, config.YEAR_END
...
def _crops_reduce(comids: np.ndarray, res: pd.DataFrame, years: list[int]) -> pd.DataFrame:
    ...
    `{year}_frac` and `{year}_unique` are per-catchment arrays of the codes present and their share of that catchment's covered area; `{year}_count` is the covered area in pixel units. Multiplying gives area per code, which `class_index` then folds into the 8 columns. VALUES ARE AREAS, NOT TALLIES, and fractions are deliberately NOT stored -- a fraction cannot be summed along the network.
    """
...
def _crops_closure(df: pd.DataFrame, cov: pd.DataFrame) -> tuple[bool, str]:
    """Per (comid, year): the class areas must sum to the covered area -- the reduce fold, audited."""
    cls = [f"{c.lower()}_px" for c in cdl_classes.CLASSES]
    gap = (df[cls].sum(axis=1) - df.n_px).abs()
    bad = int((gap > CLOSURE_TOL * df.n_px.clip(lower=1)).sum())
    return bad == 0, f"{len(df):,} rows, {bad} with class areas not closing on n_px"
...
def _weights_closure(df: pd.DataFrame, cov: pd.DataFrame) -> tuple[bool, str]:
    ...
    ok = not orphan and len(rel) > 0 and med <= WEIGHTS_TOL
...
def main(force: bool = False, only: str | None = None, limit: int | None = None,
    ...
    if "dams" in want:
        print("\n[d5.4] dams  (NID points -> containing catchment, and -> impounded reach)")
        t = dams.table(force=force)
        if t is None:
            print("  NID is not on disk; skipped (run the a3 attributes branch)")
        else:
