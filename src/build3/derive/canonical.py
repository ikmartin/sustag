...
def main(sensors: pd.DataFrame | None = None, force: bool = False,
    ...
    if tallies:
        ...
        agg = t.groupby("channel").agg(sensors=("site_uid", "nunique"), days=("n_days", "sum"),
                                       clamped=("n_clamped", "sum"), nulled=("n_nulled", "sum"))
        print(f"\n  {'channel':20s}{'sensors':>8s}{'site-days':>12s}{'clamped':>9s}{'nulled':>8s}")
        for ch, r in agg.iterrows():
            print(f"  {ch:20s}{r.sensors:>8d}{r.days:>12,}{r.clamped:>9,}{r.nulled:>8,}")
