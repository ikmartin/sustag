"""The runner. Layers are the architecture; stages are the targets.
    ...
    python -m src.build3.run acquire            # Layer A alone -- the daily-cadence entry point
    python -m src.build3.run build              # full A -> D -> P against the pinned snapshot
    python -m src.build3.run d3 [--single]      # from a stage onward, or exactly that stage
    python -m src.build3.run verify [stage] [--deep]
...
STAGES: dict[str, tuple[str, str, str]] = {
    "a1": ("seed discovery + seed basins", "src.build3.acquire.seed", "A"),
    "a2": ("census (AOE-scoped)", "src.build3.acquire.census", "A"),
    "a3": ("bulk fetch", "src.build3.acquire.bulk", "A"),
    "a4": ("verification + drift", "src.build3.acquire.reconcile", "A"),
    "d1": ("extent", "src.build3.derive.extent", "D"),
    "d2": ("canonicalisation", "src.build3.derive.canonical", "D"),
    "d3": ("sites", "src.build3.derive.sites", "D"),
    "d4": ("basins + graphs", "src.build3.derive.basins_graphs", "D"),
    "d5": ("covariates", "src.build3.derive.products", "D"),
    "p1": ("publication", "src.build3.publish.publish", "P"),
}
ORDER = tuple(STAGES)
