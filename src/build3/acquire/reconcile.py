...
ONE REPORT, because the ancestor's verification was three functions with three output shapes and no consumer. Every section returns rows into a single frame written beside the snapshot; a gap that is not named here does not exist as far as the build can tell, which is precisely how WQP0016 stayed invisible for two months.

DRIFT IS ADJUDICATED, NOT ABSORBED. `diff_snapshots` classifies every manifest change; the expected classes (trailing revisions, partition appends, additions) pass, and the conflicting ones -- an authority changed something already relied upon -- queue to the drift ledger and GATE. Both directions have precedent: a repaired export is drift to accept, a malformed registry row is drift to reject. The first post-adoption diff carries `adoption_amnesty`: adopted files have unknown fetch provenance, and flooding the queue with unanswerable rows would teach the reviewer to stop reading it.
...
# The trailing window inside which USGS revisions are EXPECTED. A revision beyond it means approved
# data changed -- a conflict, not a routine update.
...
def reconcile_sources() -> pd.DataFrame:
    ...
    IWQIS has three authorities -- the registry (location), the measures table (deployments), the export (observations) -- and the pipeline read only two of them for months while `measures.csv` and `params.csv` sat beside them carrying declared units, instruments and spans. All pairs checked, both directions; the acknowledged set lives in the registry-gaps ledger.
    """
    rows = []
    reg = iwqis._registry()
    ...
    if files:
        ...
        # WITHIN THE NAMESPACE, because the namespace rule is already this source's scope filter and a
        # reconciliation across it compares things that were never meant to correspond. The export carries
        # UNKNOWN, ARS0001, SCN0001/2, SCNOOO1, WW00001/2, bare numbers; the 2026-06 registry happened to
        # hold rows for them, so the mismatch was invisible, and the moment IIHR shipped a registry scoped
        # to its own stations all eight became "gaps" that are not gaps. A registry that scopes itself
        # correctly must not produce a queue.
...
def colocated_pairs(sensors: pd.DataFrame) -> pd.DataFrame:
    ...
    Declared beats distance -- where the declaration survives measurement. MPCA's site list names the USGS gage a station sits on, and all 13 of its checkable assertions land within 300 m (median 34.6 m), so it is kept. IWQIS's `colloc_uid` did not survive: of 48 declarations, 41 were checkable, and the 32 true ones were ALL found by proximity anyway while its 9 unique contributions were the ones geometry contradicts -- `WQS0065` naming a gage 18.4 km away, `WQS0066` 16.6 km. It added no correct pair and nine wrong ones, IIHR retired the column in the 2026-08 registry, and it is not read here. A declaration is evidence to be verified, not a fact to be trusted.
    ...
    mp = config.RAW / "water" / "MPCA_CSG" / "mpca_csg_sites.csv"
    if mp.exists():
...
def compare(uid_a: str, uid_b: str, channel: str) -> dict | None:
    ...
    if a is None or b is None:
        return None
    sa = registry.extract(a, contracts.split_uid(uid_a)[0], channel)
    sb = registry.extract(b, contracts.split_uid(uid_b)[0], channel)
...
def diff_snapshots(parent_id: str, child_id: str) -> pd.DataFrame:
    ...
    amnesty = str(snapshot.header(parent_id).get("notes", "")).startswith("adoption")
    ...
    changed = both[(ra["size"].values != rb["size"].values)
                   | (ra.mtime_ns.values != rb.mtime_ns.values)]
    now = pd.Timestamp.now()
    for rel in changed:
        if amnesty:
            rows.append(dict(relpath=rel, drift="adoption_amnesty"))
            continue
        grew = int(b.loc[rel, "size"]) > int(a.loc[rel, "size"])
        recent = (now - pd.Timestamp(b.loc[rel, "fetched_at"])).days <= REVISION_WINDOW_DAYS
        if grew and recent:
            rows.append(dict(relpath=rel, drift="extended"))
        elif recent:
            rows.append(dict(relpath=rel, drift="revised_in_window"))
        else:
            rows.append(dict(relpath=rel, drift="revised_out_of_window"))
