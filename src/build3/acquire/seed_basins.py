...
def fetch_authority_basins(sensors: pd.DataFrame | None = None, force: bool = False,
                           workers: int = _WORKERS) -> pd.DataFrame:
    """NLDI's `nwissite` delineation per USGS sensor CARRYING A RECORD, cached one file per uid.

    THE FETCH LIVES HERE, NOT IN D4 -- derivation is a pure function and this is a network call. Same bootstrap pattern as the census and the waterbody pull: reading the previous build's state (which sensors canonicalised a record) to decide WHAT to fetch is sanctioned; a first pass with no canonical store fetches nothing and the next one catches up.

    FAILURES ARE CACHED TOO. NLDI's index simply omits some registrations (the Delaware at Chester, the Susquehanna at Columbia), and refetching a known 404 every pass is how a daily-cadence acquisition run gets slow. A cached failure file carries `status` and no geometry; `--force` is the way to ask again.
    """
    import geopandas as gpd

    if sensors is None:
        if not config.SENSORS_PATH.exists():
            print("  no sensors table yet -- nothing to fetch (first-build no-op)")
            return pd.DataFrame(columns=["site_uid", "status"])
        sensors = pd.read_parquet(config.SENSORS_PATH)

    def has_record(uid: str) -> bool:
        return (config.WATER_CANONICAL / f"{schema.uid_to_filename(uid)}.parquet").exists()

    config.ACQ_AUTHORITY_BASINS.mkdir(parents=True, exist_ok=True)
    usgs = sensors[(sensors.source == "USGS")]
    usgs = usgs[[has_record(u) for u in usgs.site_uid]]
    todo, out = [], []
    for r in usgs.itertuples():
        p = config.ACQ_AUTHORITY_BASINS / f"{schema.uid_to_filename(r.site_uid)}.parquet"
        if p.exists() and not force:
            out.append({"site_uid": r.site_uid, "status": "cached"})
        else:
            todo.append((r.site_uid, str(r.source_site_id), p))
    print(f"  authority basins: {len(out)} cached, {len(todo)} to fetch "
          f"({len(usgs)} USGS sensor(s) with records)", flush=True)
    if not todo:
        return pd.DataFrame(out, columns=["site_uid", "status"])
    ...
    def one(args):
        uid, nid, path = args
        try:
            geom = nldi.get_basins(nid).geometry.iloc[0]
            status = "ok"
        except Exception:
            geom, status = None, "no_nldi_basin"
        gpd.GeoDataFrame({"site_uid": [uid], "status": [status]},
                         geometry=[geom], crs=4326).to_parquet(path)
        return {"site_uid": uid, "status": status}
    ...
    df = pd.DataFrame(out)
    print(f"  authority basin outcomes: {df.status.value_counts().to_dict()}")
    return df


def read_authority_basin(uid: str):
    """The cached NLDI polygon for one sensor, or `(None, reason)` -- a PURE READ for D4.

    Returns `(geometry, status)`: `ok` with a polygon, `no_nldi_basin` for a recorded miss, `not_fetched` when acquisition has not asked yet -- three different answers, and only the last one is fixed by rerunning a3.
    """
    import geopandas as gpd

    p = config.ACQ_AUTHORITY_BASINS / f"{schema.uid_to_filename(uid)}.parquet"
    if not p.exists():
        return None, "not_fetched"
    g = gpd.read_parquet(p)
    if not len(g) or g.geometry.iloc[0] is None:
        return None, str(g.status.iloc[0]) if len(g) else "no_nldi_basin"
    return g.geometry.iloc[0], "ok"
