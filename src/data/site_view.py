...
Unifies the old build_grid (real site) and build_grid_from_basin (virtual/deploy) into one function over the canonical grid_global. A "site" is the set of grid_global cells whose polygon intersects the basin, relabelled with a basin-local node_id, plus two basin-relative columns:
dist_to_sensor (D8 flow distance from the cell to the sensor's outlet) and frac_cell_in_basin (fraction of the cell inside the basin).

The one runtime module carrying geo deps (the D8 flow raster via src.data.d8, the geopandas intersection). access.py re-exports build_site_view so callers get it off the unified surface; deploy/ imports it to build a virtual site's grid live from an arbitrary pin.
...
def _grid_node_distances(grid: gpd.GeoDataFrame, field: np.ndarray) -> np.ndarray:
    """Flow distance (m) to the sensor for each cell. Pass 1 routes each node's centroid through the D8 field (with straddler recovery); pass 2 fills any still-NaN node from the nearest resolved node B: dist(A) = dist(B) + |centre(A) - centre(B)| (EPSG:5070 metres)."""
    dist = np.array([
...
def build_site_view(basin: gpd.GeoDataFrame, sensor_lat: float, sensor_lon: float, label: str = "") -> gpd.GeoDataFrame:
    ...
    `basin` is any-CRS (reprojected to EPSG:5070 here). Works for a real site (basin from access.get_basin) or a virtual/ungauged site (NLDI basin from a drop point). Returns the per-site grid in the schema above, or an empty GeoDataFrame if no cell intersects the basin.
    """
    gg = _grid_global()
