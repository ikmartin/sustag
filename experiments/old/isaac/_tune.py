...
from experiments.old.isaac.cook import _pool, _features, _target, basin_groups, _grouped_oof, _score
...
def tune_task(task, recipe, target_col, metric, sites, depths, lrs, regular, n_splits, ceiling):
    ...
    for i, override in enumerate(grid, 1):
        ...
        lofo = (
            _score(y, _grouped_oof(X, y, fam_grp, task, n_splits, **cfg), task)[metric] if n_fam >= 2 else float("nan")
        )
...
def main():
    ...
    ap.add_argument(
        "task", nargs="?", default="both", choices=["reg", "clf", "both"], help="which target to tune (default: both)"
    )
    ap.add_argument(
        "--module", default="recipes3", help="module to import recipe_REG/recipe_CLF from (default: recipes3)"
    )
    ap.add_argument("--depths", type=_csv_ints, default=[3, 4, 5, 6], help="max_depth values (default 3,4,5,6)")
    ap.add_argument(
        "--lrs", type=_csv_floats, default=[0.01, 0.02, 0.05], help="learning_rate values (default 0.01,0.02,0.05)"
    )
    ap.add_argument(
        "--regular",
        action="store_true",
    ...
    ap.add_argument(
        "--fast", action="store_true", help="smoke-test: 12 sites, depths 3,4, lrs 0.02,0.05, n_estimators ceiling 800"
    )
    args = ap.parse_args()
