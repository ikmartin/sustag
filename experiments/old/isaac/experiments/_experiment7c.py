"""_experiment7c -- CLASSIFIER version of _experiment7.

Same A/B/C feature ladders (covariates -> +own AR -> +cross-site climatology), but the
TARGET is the binary 'violation' (daily nitrate >= 10 mg/L). Continuous nitrate is never a
feature (it would leak the label); n_daily is used only for the spine. Evaluated
individual-site via compare_fleet (cook_one per site, scored by AUC).
"""
...
from experiments.old.isaac.cook import *
...
def _covariates_cached(site, edges=()):
    """weather + crops/surplus (exp-decay buckets) + pure calendar. Cached; parts returned as
    an IMMUTABLE tuple. n_daily (continuous) is returned only for use as the spine."""
...
def recipe_B(site):
    """A + the site's OWN past nitrate (lagged; causal, no same-day leak)."""
    n_daily, parts = _covariates(site)
    own = [lagged_sensor_nitrate([site], shift=k) for k in (1, 2, 3, 7, 14, 30)]
    return merge_on_date([_violation(site), *parts, *own], spine=n_daily.index)
...
def recipe_C(site):
    ...
    return merge_on_date([_violation(site), *parts, *clim, *neigh], spine=dates)
...
def main(sites=None, extra=False):
    ...
    results = compare_fleet(recipes, sites=sites, target_col="violation", task="clf", extra_importance_test=extra)
