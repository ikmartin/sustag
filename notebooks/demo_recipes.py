...
A/D feature sets cropped to a fixed date window, but the TARGET is the binary 'violation' (daily nitrate >= 10 mg/L). Continuous nitrate is never a feature; n_daily is only the spine. Cross-site evaluation via compare_many (scored by AUC / PR-AUC / Brier).
"""
...
def _covariates_cached(site, edges=()):
    """weather + crops/surplus (exp-decay buckets) + pure calendar. Cached (immutable tuple). n_daily (continuous) is returned only for use as the spine, never as a feature."""
...
def preet_recipe(site, lam: int = PREET_LAM, roll_window: int = PREET_ROLL_WINDOW):
    """Preet's BREACH-CLASSIFIER feature geometry from **testing_stuff.ipynb** (her later, classifier notebook -- NOT the EDA_and_modeling regression notebook), rebuilt on the refactored data layer for `plot_pr_curves(preet_recipe)` / `run_cv(preet_recipe, task="clf")`. Mirrors testing_stuff's choices:
    ...
    Faithful to the *approach* in testing_stuff.ipynb, not a byte-exact port (its multi-scale _short/_med/_long variants, seasonal interactions, leaching_risk, etc. are not reproduced). For Preet's EDA_and_modeling *regression* baseline instead, see demo_model.preet_xgb_baseline."""
