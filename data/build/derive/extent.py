...
def _prior():
    ...
    legacy = config.REPO / "src" / "data" / "derived" / "aoe.parquet"
    if legacy.exists():
        return gpd.read_parquet(legacy), " (the predecessor build's -- stamp continuity across the rewrite: the adopted network/weather/attribute stores were cut against this stamp, and re-stamping an unchanged region would invalidate all of them for nothing)"
    return None, ""
