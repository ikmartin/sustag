...
THE SCRUB HAPPENS INSIDE `registry.extract(scrub="full")`, per native cell, before the combine -- sentinels and range violations are counted at the grain they occur at, and the tally closes: `n_kept = n_cells - n_sentinel - n_below_lo - n_above_hi`, clamped cells kept at 0.
...
from .. import io as bio, registry as params
...
def channel_frame(obs: pd.DataFrame, source: str, channel: str) -> tuple[pd.DataFrame | None, dict]:
    """Daily statistics for one channel, plus the cell-grain scrub tally. `(None, tally)` when the site does not carry it."""
    tally = dict(channel=channel, n_cells=0, n_sentinel=0, n_below_lo=0, n_clamped=0,
                 n_above_hi=0, n_kept=0, n_days=0, preaggregated=False)
    s, t = params.extract(obs, source, channel, scrub="full")
    tally.update({k: t[k] for k in ("n_cells", "n_sentinel", "n_below_lo", "n_clamped", "n_above_hi")})
    tally["n_kept"] = t["n_cells"] - t["n_sentinel"] - t["n_below_lo"] - t["n_above_hi"]
