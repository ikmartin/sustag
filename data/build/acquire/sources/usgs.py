...
NAME = "USGS"
DECLARES_SPANS = True       # series metadata carries per-parameter begin/end dates
...
# ---- raw-GeoJSON coordinate capture ---------------------------------------------------------------
# The client library parses the monitoring-locations response to floats in memory, so the coordinate
# TEXT -- the only carrier of declared precision -- never reaches disk through it. This path re-requests
# the same OGC items endpoint directly, ARCHIVES the raw response, and reads the coordinates with
# `parse_float=str` so the printed decimal places survive. Failure costs dp (null, visible to verify),
# never the discovery row.
_OGC_ITEMS = "https://api.waterdata.usgs.gov/ogcapi/v0/collections/monitoring-locations/items"


def _geojson_batch(ids: list[str]) -> dict | None:
    """Fetch, archive, and parse one batch of monitoring locations as raw GeoJSON with STRING floats."""
    import hashlib
    import json
    ...
    for attempt in range(5):
        try:
            r = requests.get(
                _OGC_ITEMS,
                params={"f": "json", "id": ",".join(ids), "limit": max(len(ids), 1)},
                headers={"X-Api-Key": os.environ.get("API_USGS_PAT", ""),
                         "Accept": "application/geo+json"},
                timeout=120)
        except Exception as e:
            print(f"    coord_dp batch: {type(e).__name__}: {str(e)[:60]} (attempt {attempt + 1})")
        ...
        print(f"    coord_dp batch: HTTP {r.status_code} (attempt {attempt + 1}); waiting {wait:.0f}s")
    ...
    if r is None or r.status_code != 200:
        print(f"    coord_dp batch failed; precision stays null for {len(ids)} id(s)")
        return None
    config.ACQ_USGS_GEOJSON.mkdir(parents=True, exist_ok=True)
    tag = hashlib.sha256(",".join(sorted(ids)).encode()).hexdigest()[:16]
    out = config.ACQ_USGS_GEOJSON / f"ml_{tag}.json"
    tmp = out.with_name(out.name + ".tmp")
    tmp.write_text(r.text)
    os.replace(tmp, out)
    return json.loads(r.text, parse_float=lambda x: x)


def coord_dp_for(ids: list[str]) -> pd.DataFrame:
    """(monitoring_location_id, coord_dp_lat, coord_dp_lon) for many ids, from archived raw responses."""
    rows = []
    for i in range(0, len(ids), _ML_BATCH):
        doc = _geojson_batch(ids[i:i + _ML_BATCH])
        if not doc:
            continue
        for feat in doc.get("features", []):
            coords = (feat.get("geometry") or {}).get("coordinates") or [None, None]
            lon_t, lat_t = (coords + [None, None])[:2]
            rows.append(dict(monitoring_location_id=str(feat.get("id", "")),
                             coord_dp_lat=_dp_of_text(lat_t), coord_dp_lon=_dp_of_text(lon_t)))
    return pd.DataFrame(rows, columns=["monitoring_location_id", "coord_dp_lat", "coord_dp_lon"])


def _dp_of_text(t) -> object:
    """Decimal places of one JSON-number-as-string; <NA> for anything else."""
    s = str(t or "").strip()
...
def discover(census: bool = True, cover=None, clip=None) -> pd.DataFrame:
    ...
    dp = coord_dp_for(per.monitoring_location_id.tolist())
    dp_map_lat = dp.set_index("monitoring_location_id").coord_dp_lat if len(dp) else {}
    dp_map_lon = dp.set_index("monitoring_location_id").coord_dp_lon if len(dp) else {}
    ...
    out = pd.DataFrame({
        ...
        "regime": "continuous",
        "coord_dp_lat": m.monitoring_location_id.map(dp_map_lat),
        "coord_dp_lon": m.monitoring_location_id.map(dp_map_lon),
        "pcodes": m.pcodes,
...
def register_one(native_id: str) -> pd.DataFrame:
    ...
    dp = coord_dp_for([f"USGS-{native_id}"])
    ...
    out = pd.DataFrame({
        ...
        "regime": "continuous",
        "coord_dp_lat": [dp.coord_dp_lat.iloc[0] if len(dp) else pd.NA],
        "coord_dp_lon": [dp.coord_dp_lon.iloc[0] if len(dp) else pd.NA],
...
def fetch(native_id: str, start=None, end=None, channels: str | None = None) -> pd.DataFrame | None:
    ...
    CONCURRENCY LIVES IN `_native` AND NOWHERE ELSE: the windows of one site's pull run `_CONCURRENT` at a time while the library's own fan-out is pinned to 1 (`_auth`), so that constant is the total simultaneous load this module puts on the API.
    ...
    if tier == "dv":
        ...
        start = pd.Timestamp(start or config.collection_start().isoformat(), tz="UTC")
    ...
    # An empty nitrate window does NOT lose the site's other channels: a nitrate-declared station whose
    # nitrate pull returns nothing still carries discharge, stage, turbidity -- under the all-channels
    # census those are the point. Only a site returning nothing AT ALL comes back None.
    dv = _daily_values(native_id, others, start, end)
    served = set()
    if dv is not None and len(dv):
        served = set(dv.columns)
        dv_suffixed = dv.add_suffix("_dv")
        out = out.join(dv_suffixed, how="outer") if out is not None else dv_suffixed
    ...
    if missing:
        ...
        if nat is not None:
            out = out.join(nat, how="outer") if out is not None else nat
    return out
