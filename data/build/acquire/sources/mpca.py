...
NAME = "MPCA"
DECLARES_SPANS = False

_DIR = config.RAW / "water" / "MPCA_CSG"
...
def discover(clip=None) -> pd.DataFrame:
    ...
    # Coordinate text re-read as strings for declared precision. Caveat recorded: mpca_csg_sites.csv was
    # itself written from API JSON by the fetch experiment, so trailing zeros are already gone and the
    # captured dp is a LOWER BOUND on what MPCA declares.
    txt = pd.read_csv(_SITES, dtype=str)
    dp_lat = base.text_dp(txt["lat"]) if "lat" in txt.columns else pd.NA
    dp_lon = base.text_dp(txt["lon"]) if "lon" in txt.columns else pd.NA
    out = pd.DataFrame({
...
# The -99999 fill value this adapter once replaced at fetch time now lives in `registry.SENTINELS`:
# a sentinel is a registry fact, masked at canonicalisation where it is COUNTED, and the native
# parquet keeps the raw value so the scrub is auditable and `impossible_runs` can see the fill runs.
