...
def snap(sites: pd.DataFrame, flowlines, catchments=None, vaa=None) -> pd.DataFrame:
    ...
    # A registration with no coordinates (the coordinate-sentinel class) cannot snap and is not asked
    # to: its row comes back `snap_status="no_location"` with every geometric column null, so "snapped
    # or named-failed" is a partition rather than a hope. The nearest-neighbour fallback below means a
    # LOCATED point can never fail to find a reach; no_location is the one genuine failure state.
    located = sites.lat.notna() & sites.lon.notna()
    if not located.all():
        noloc = sites.loc[~located, ["site_uid"]].copy()
        noloc["snap_status"] = "no_location"
        done = snap(sites[located].reset_index(drop=True), flowlines, catchments, vaa)             if located.any() else pd.DataFrame(columns=["site_uid", "snap_status"])
        return pd.concat([done, noloc], ignore_index=True)
    ...
    if MIN_STREAM_ORDER is not None:
        ...
        oc = low.get("streamorde")     # case-INsensitive: the exact trap _VAA_COLS exists to prevent, no longer dormant
        if oc is not None:
