...
The map panel writes the user's current selection ("region of interest") to the `region-geom` store as a GeoJSON geometry — either a Point (point-select mode) or a Polygon (rectangle/polygon draw mode). The functions here turn that selection into whatever shape downstream consumers need.
"""
...
def delineate_basin_v1_for_pin(lat: float, lon: float, site_uid: str = "pin", timeout: int = 60) -> dict:
    ...
    AUTHORITATIVE COMID resolved from the snapped reach. No reported_area is passed -- a map pin has none -- so this is the pure-nearest rule, identical to how a new deploy site would be delineated.

    A COMID must never be inherited between delineations; its TOT_/CAT_ attributes describe its own reach's catchment. Pass the real ``site_uid`` when saving a correction; "pin" is for live preview.

    Returns a GeoJSON FeatureCollection dict (EPSG:4326). Raises ValueError outside the flowlines extent or off the NHD network.
    """
...
def delineate_basin_v2_for_pin(lat: float, lon: float, site_uid: str = "pin", timeout: int = 60) -> dict:
    ...
    Returns whichever catchment polygon the point falls inside, which is NOT necessarily the reach the pin sits on (see _make_basins for the mainstem-divergence failure). Shown alongside v1 so a reviewer can see when the two methods disagree.
    """
...
def delineate_basin_v3_for_pin(lat: float, lon: float) -> dict:
    ...
    Same 500 m flow-direction raster and BFS flood-fill as the builder's basin3. The direction array is loaded once and cached. Raises ValueError outside the Iowa raster domain.
    """
...
def normalize_for_forecast(region_geojson, buffer_deg=DEFAULT_POINT_BUFFER_DEG):
    ...
    Points (from point-select mode) are buffered into a small disk so the model always receives an area. Polygons (from rectangle/polygon draw mode) are passed through unchanged.
    """
...
def get_downstream_sites(region_geojson, sites_df):
    ...
    network dataset that isn't integrated yet. Returns an empty list so callers can be written against the final interface now.
