...
Reuses src.features.recipes.build_feature_frame (the exact assembly the gauged recipes use) with a weather-derived spine = the TARGET_YEAR daily dates. The SiteData.weather spans TARGET_YEAR ±2 months, so the trailing rolling/lag features have lead-in; trimming the spine to the year keeps that buffer for lookback while emitting only in-year rows.
"""
...
def target_year_spine(site_data, target_year: int = TARGET_YEAR) -> pd.DatetimeIndex:
    """Daily DatetimeIndex over `target_year`, taken from the site's weather dates (which span target_year ±2 months). Trimming to the year drops the lookback buffer from the OUTPUT rows while keeping it available to the rolling/lag feature computations."""
...
def virtual_recipe(site_data, task: str = "reg", target_year: int = TARGET_YEAR) -> pd.DataFrame:
    ...
    Columns match _best_features_REG / _best_features_CLF (recipe_REG/_CLF minus the target). Align to a trained model's feature_names before scoring (see predict).
    """
